...
func (b *Bindings) GenerateBackendJS(targetfile string) error {
	...
	var outputBinds bytes.Buffer

	outputBinds.WriteString("\n")
...
function mapCall(packageName, structName, methodName) {
  bindMap[packageName] = bindMap[packageName] || {};
  bindMap[packageName][structName] = bindMap[packageName][structName] || {};
  bindMap[packageName][structName][methodName] = null
}

function bindCalls() {
  window.wails.SetBindings(JSON.stringify(bindMap))
}
	...
	sortedPackageNames.Each(func(packageName string) {
		...
		sortedStructNames.Each(func(structName string) {
			...
			sortedMethodNames.Each(func(methodName string) {
				...
				outputBinds.WriteString(fmt.Sprintf("mapCall(\"%s\", \"%s\", \"%s\");\n", packageName, structName, methodName))
				output.WriteString("\n")
...
export default go;`)
	...
	outputBinds.WriteString("bindCalls()\n")
	output.Write(outputBinds.Bytes())
